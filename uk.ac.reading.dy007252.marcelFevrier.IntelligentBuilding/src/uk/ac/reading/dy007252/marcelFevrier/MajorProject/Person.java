...
public class Person {
	...
	private void moveTowards(Point d) {
		int dx = 0;
		int dy = 0;
		
		if (this.position.getX() < d.getX()) dx = 1;
		else if (this.position.getX() > d.getX()) dx = -1;
		
		if (this.position.getY() < d.getY()) dy = 1;
		else if (this.position.getY() > d.getY()) dy = -1;
	...
	public void move() {
		...
		this.moveTowards(this.path.get(0));
